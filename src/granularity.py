...
def store_results(results_dir, granularity, durations_pdu, durations_redfish, durations_rapl):
    ...
    with open(csv_file, mode='a', newline='') as f:
        writer = csv.writer(f)
        if f.tell() == 0:
            writer.writerow(["GRANULARITY", "PDU_DURATION", "Redfish_DURATION", "RAPL_DURATION"])
...
def run_granularity_tests(results_dir):
    for granularity in granularity_values:
        ...
        store_results(results_dir, granularity, durations_pdu, durations_redfish, durations_rapl)
